...
class Module {
    ...
    addExport(node) {
        if (node instanceof ExportDefaultDeclaration) {
            // export default foo;  //导出基础模块
            let vName = node.variable.getAssignedVariableName();
    ...
    preRender(options, inputBase) {
        ...
        for (const module of this.orderedModules) {
            ...
            if (module.isIncluded()) {
                ...
                if(options.jonlyImport!=undefined && options.jonlyImport!=""){
                    let nstr = source.filename.replace(/\\/g, '/');
                    if(nstr != options.jonlyImport){
                        continue;
                    }
                }

                let ji = options.jcustExternal.indexOf(source.filename);
                if(ji != -1){
                    continue;
                }
...
function normalizeOutputOptions(inputOptions, rawOutputOptions, hasMultipleChunks, outputPluginDriver) {
    ...
    outputOptions.jcustExternal = rawOutputOptions.jcustExternal||[];
    outputOptions.jonlyImport = rawOutputOptions.jonlyImport;
