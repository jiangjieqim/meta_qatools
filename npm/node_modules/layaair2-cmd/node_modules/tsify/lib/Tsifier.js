...
module.exports = function (ts) {
	...
	function parseOptions(opts, bopts) {
		...
		// $LayaFlag
		var trueCurDir = path.join(path.dirname(parsed.options.configFilePath));
		var tureRootDir = path.parse(trueCurDir).root;
		parsed.options.rootDir = path.relative(trueCurDir, tureRootDir);
	...
	function Tsifier(opts, bopts) {
		...
		// $LayaFlag 修改options的outDir
		let trueRoot = path.parse(self.opts.configFilePath).root;
		let outDirRoot = path.parse(self.opts.outDir).root;
		self.opts.outDir = self.opts.outDir.replace(outDirRoot, trueRoot);
